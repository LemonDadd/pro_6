...
for i in range(5):
    ...
    if r.status_code == 200:
        ...
        print(f"  请求 {i+1}: {r.status_code} - {r.text[:100]}")
...
print("=" * 60)
print("注意：此测试需要 30s+，故意渲染超大文档触发超时")
print("按 Ctrl+C 可跳过此测试\n")
...
try:
    ...
    r = client.post(f'{BASE_URL}/v1/render/sync',
        ...
        json={
            'markdown': huge_md,
            'theme': 'github',
            'options': {'toc': True, 'codeHighlight': False, 'mermaid': False}
        }
    )
